...
filters %<>% 
  drop_na() %>%
  arrange(Colname)
filter_groups <- unique(filters$Class)
  ...
  filters %>% 
    ...
    lapply(., function(var) {
      if (classes[[var]] %in% c("character","factor")) {
        ...
        min <- min(combined_df_list[[input$specialty]][[var]], na.rm = T)
        max <- max(combined_df_list[[input$specialty]][[var]], na.rm = T)
        sliderInput(var,var,min,max,c(min,max))
      }
      }
    )
})
#   pickerInput("test","ID", choices=unique(combined_df_list[[input$specialty]]$ID),selected = unique(combined_df_list[[input$specialty]]$`ID`),  multiple = T, options=list(`liveSearch` = T, `actions-box` = TRUE,size = 10,
...
<!-- ```{r} -->
<!-- py$models$`Internal Medicine - Categorical Track`$recommend_from_interactions(observed_items=list('1400531051C'),k=as.integer(1000))$to_dataframe()[,c("ID","rank")] %>%  -->
<!--   as.data.frame(.) %>%  -->
...
<!-- ```{r} -->
<!-- renderPrint({ -->
<!--   py$models[[input$specialty]]$recommend_from_interactions(observed_items=as.list(selectedIDs()),k=as.integer(1000))$to_dataframe()[,] -->
<!-- }) -->
<!-- ``` -->
...
output$table <- DT::renderDT({
  ...
  # create and style DR
  val <- to_join()
  vis <- c("Range","Residency program name", "City", "State", "Overall rank","Program","Freida","AAMC","Doximity",
           "# Residents on Duty Year 1","Current residents Step 1 range")
  df %<>%
  ...
  colname_change_vec <- as.numeric(lapply(names(colname_change_list), function(i) which(colnames(df) %in% i)))
