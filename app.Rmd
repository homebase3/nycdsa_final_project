...
output$gg <- renderPlotly({
  ...
  ggplot(data = data.frame(x = c(lbound,ubound)), aes(x)) +
    ...
    geom_text(data=data.frame(x=actual,y=dnorm(actual)), aes(x=x, y=y, label=paste0("You: ",scales::ordinal(score*100), " percentile")), color="blue", nudge_x = if_else(actual>0,1,-1)) +
