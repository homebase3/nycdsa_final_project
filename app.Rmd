...
```{r, context="render"}
valueBoxOutput("valuebox_reach") %>% withSpinner()
```
...
```{r, context="render"}
valueBoxOutput("valuebox_target") %>% withSpinner(type=8)
```
...
output$table <- DT::renderDT({
  ...
  colnames(df)[to_hide1] %>% 
    str_sub(.,1,-3) %>% 
    sapply(.,function(i) which(colnames(df) == i)) %>% 
    as.character(.) -> to_color
  ...
  datatable(df, escape = F,
    ...
    formatStyle(.,columns=to_hide1,valueColumns=to_color,color = styleEqual(c(NA),c("red")))
