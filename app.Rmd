...
steps2 <-data.frame(element = c("#table", ".navbar ","#table","#table","#table","#section-row-2",".btn",".section-inputs2","#UIProgram_basics","#UIProgram_basics","#UIProgram_basics","#UIdownload"
                               ),
...
output$valuebox <- renderValueBox({
  ...
  score %>% 
    round(.) %>% 
    scales::ordinal(.) %>% 
    tags$p(., style = "font-size: 175%; color: #fff") %>% 
    valueBox(., caption = tags$p(paste0("percentile overall in ",input$specialty),style = "font-size: 100%; color: #fff"), icon = 'fa-star', color = col, href = NULL)
})
```
...
output$valuebox_reach <- renderValueBox({
    valueBox(tags$p(difficulty_counts()[["Reach"]], style = "font-size: 100%; color: #fff"), caption = tags$p(paste0("target programs selected"), style = "font-size: 100%; color: #fff"), color = "info", href = NULL)
})
```
...
output$valuebox_target <- renderValueBox({
    valueBox(tags$p(difficulty_counts()[["Target"]], style = "font-size: 100%; color: #fff"), caption = tags$p(paste0("target programs selected"),style = "font-size: 100%; color: #fff"), color = "warning", href = NULL)
})
```
...
output$valuebox_safer <- renderValueBox({
    valueBox(tags$p(difficulty_counts()[["Safer"]], style = "font-size: 100%; color: #fff"), caption = tags$p(paste0("safer programs selected"),style = "font-size: 100%; color: #fff"), color = "success", href = NULL)
})
```
...
output$table <- DT::renderDT({
  ...
  df <- combined_df_list[[input$specialty]] %>% 
    arrange(Overall_rank) %>% 
    mutate(Range = if_else(Overall_score > reach, "Reach", if_else(Overall_score > safer, "Target","Safer")), .before=1) %>%  
    mutate(`Program website` = if_else(is.na(`Program website`),as.character(NA),paste0('<a style="font-size:75%" href="', `Program website`,'" target="_blank" class="btn btn-1">Link</a>'))) %>% 
    mutate(`Freida` = if_else(is.na(`ID`),as.character(NA),paste0('<a style="font-size:75%" href="https://freida.ama-assn.org/program/', `ID`,'" target="_blank" class="btn btn-2">Link</a>'))) %>% 
    mutate(`AAMC link` = if_else(is.na(`AAMC link`),as.character(NA),paste0('<a style="font-size:75%" href="', `AAMC link`,'" target="_blank" class="btn btn-3">Link</a>'))) %>%
    mutate(`Doximity link` = if_else(is.na(`Doximity link`),as.character(NA),paste0('<a style="font-size:75%" href="', `Doximity link`,'" target="_blank" class="btn btn-4">Link</a>'))) %>% 
  ...
  datatable(df, escape = F,
    ...
    formatStyle(.,columns = 1:ncol(df), fontSize = '75%')
