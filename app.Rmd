...
/*Remove grey line on page from bs4 error in flexdashboard*/
hr {
...
/*Reduce vertical spacing between dataTables elements*/
...
```{python import_models, context="setup", include = FALSE, cache=T}
...
steps1 <- data.frame(
  element = c("#specialty-selectized", "#section-applicant_sidebar", "#section-overall-score", "#step1Gauge", ".navbar"),
  intro = c(
    "To get started, select your desired specialty.",
...
steps2 <- data.frame(
  element = c("#table", ".navbar ", "#table", "#table", "#table", "#section-range_gauge", "#section-selectbar", "#section-selectbar", "#section-selectbar", "#section-selectbar", "#section-selectbar", "#section-selectbar", "#table", "#section-sidebar"),
  intro = c(
    "First off, you'll see a list of (nearly) every program in your specialty. Programs are bucketted into \"Reach\", \"Target\", and \"Safer\" based on (1) your competitiveness in the specialty and (2) our assessment of the program based on applicant competitiveness, research quality, teaching quality, and patient outcomes. The initial order of the programs is based on this rank list, with the top program at the top.",
    "You can find more detail on how your specialty was ranked by clicking \"FAQ\" in the Navbar.",
    ...
    "To export your list, first toggle on the \"Export toggle\" tag.",
    "Then click download in the table to download a csv.",
    "If you'd like to save your progress for future use, simply download a csv and upload here next time you visit the app."
  )
)
...
Inputs {.sidebar id="applicant_sidebar"}
------
```{r}
...
output$step1Gauge <- renderGauge({
  score_step1(input$step1, input$specialty, spec_dat, dists) %>%
    `*`(100) %>%
    round(.) %>%
    gauge(., 0, 100, label = "Percentile", sectors = gaugeSectors(
      success = c(50, 100),
      warning = c(20, 50),
      danger = c(0, 20),
...
actionButton("button", "Recommend and resort", icon("thumbs-up"))
actionButton("show", "Remove all filters", icon("broom"))
fileInput("file1", label = NULL, buttonLabel = list(icon("file-import"), "Import"), accept = c(".csv", ".xlsx"))
materialSwitch(inputId = "selected_switch", label = "Export mode", value = FALSE)
```

<details>
<summary>Program range</summary>
```{r}
pickerInput("range_filter", "Program range",
  choices = c("Reach", "Target", "Safer"),
  selected = c("Reach", "Target", "Safer"), multiple = T, options = list(`liveSearch` = T, `actions-box` = TRUE, size = 10, `selected-text-format` = "count > 3")
)
```
</details>
...
Row {data-height=50 id="range_gauge"}
-----
    ...
    for (var in vars_) {
      if (!is.null(classes[[var]])) {
        if (as.character(classes[[var]]) == "numeric") {
          ...
          updatePickerInput(session, inputId = gsub(" ", "_", var), choices = val, selected = val, choicesOpt = list(
            content = stringr::str_trunc(val, width = 15)
          ))
        }
      }
    }
...
```{r server_logic_2, context='server'}
...
observeEvent(input$file1, {
  ...
  if (ext == "csv") {
    old_select <- read_csv(file$datapath)
  } else {
    old_select <- read_excel(file$datapath, header)
  }

  old_IDs <- old_select %>%
    dplyr::select(`Residency program name`) %>%
    left_join(., combined_df_list[[input$specialty]]) %>%
    dplyr::select(ID) %>%
    drop_na(.) %>%
    .[[1]]

  ### NOTE code here adapted from below snippet for selected list update
  for (ID in old_IDs) {
  ...
  # update dummy to trigger refresh of dataTable
  dummyVal <- dummy() + 1
  dummy(dummyVal)
})
```
...
```{r server_logic_3, context='server'}
# log selections
...
output$table <- DT::renderDataTable(
  {
    ...
    df %<>%
      filter(Range %in% input$range_filter)
    if (input$selected_switch == TRUE) {
      df %<>%
        filter(ID %in% selectedIDs())
    }
    for (group in filter_groups) {
...
output$kable_factors2 <- function() {
  req(input$specialty)
  categories <- c(
    rep("Applicant competitiveness (core)", 5),
