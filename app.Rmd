...
- Programs did not provide relevant info (e.g. step scores) to the AAMC
- Auxiliary information (e.g. Medicare Hospital data) could not be joined to the underlying program data due to an inadequate match

```{r, context="server"}
output$text5 <- renderText({
  missing <- sum(is.na(dat_list[[input$specialty]]))
  total <- nrow(dat_list) * (ncol(dat_list)-2) #subtract two due to name and ID
