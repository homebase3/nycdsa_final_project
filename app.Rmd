    ...
    for (var in vars_) {
      if (!is.null(classes[[var]])) {
        if (as.character(classes[[var]]) == "numeric") {
          min<- floor(min(combined_df_list[[input$specialty]][[var]], na.rm = T))
          max <- ceiling(max(combined_df_list[[input$specialty]][[var]], na.rm = T))
          updateSliderInput(session,inputId=gsub(" ","_",var), min=mins[['Percentage of completed residents with other career plans']], max= maxs[['Percentage of completed residents with other career plans']], value=c(mins[['Percentage of completed residents with other career plans']],maxs[['Percentage of completed residents with other career plans']]))
        } else {
    ...
    for (var in vars_[2]) {
      if (!is.null(classes[[var]])) {
        if (classes[[var]] %in% c("character","factor")) {
          val4(df[[var]] %in% input[[gsub(" ","_",var)]])
          df <- df[df[[var]] %in% input[[gsub(" ","_",var)]],]
          # df %<>%
          #   filter(!!rlang::sym(var) %in% isolate(input[[gsub(" ","_",var)]]))
            # filter_at(vars(any_of(var)), any_vars((. %in% input[[gsub(" ","_",var)]])))
          } else {
          val1(input[[gsub(" ","_",var)]])
          val2(unique(df[[var]]))
          val3(var)
          val5(colnames(df))
           df <- df[df[[var]] >= input[[gsub(" ","_",var)]][1] & df[[var]] >= input[[gsub(" ","_",var)]][2],]
            # df %<>%
            #   filter(!!rlang::sym(var) >= isolate(input[[gsub(" ","_",var)]][1]) & !!rlang::sym(var) <= isolate(input[[gsub(" ","_",var)]][2]))
              # filter_at(vars(any_of(var)), any_vars((. >= input[[gsub(" ","_",var)]][1] & . <= input[[gsub(" ","_",var)]][2])))
          }
      }
    }
  }
